...
if (!$lampiran){
  ...
  $query = "UPDATE artikel set judul_artikel = '".$judul_artikel."', deskripsi_artikel = '".$deskripsi_artikel."' WHERE id = '".$id."' ";
  $sql = mysqli_query($connect, $query); // Eksekusi/ Jalankan query dari variabel $query
    
    if($sql){ // Cek jika proses simpan ke database sukses atau tidak
      // Jika Sukses, Lakukan :
      echo "<script>alert('artikel anda telah di ubah.'); window.location.href='./daftar_artikel';</script>";
      exit();
    ...
    $query = "UPDATE artikel set judul_artikel = '".$judul_artikel."', deskripsi_artikel = '".$deskripsi_artikel."', lampiran = '".$lampiran_upload."' WHERE id = '".$id."' ";
